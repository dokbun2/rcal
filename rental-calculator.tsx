...
const RentalCalculator: React.FC = () => {
  ...
  const [supplyRatePercent, setSupplyRatePercent] = useState<number>(100);
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e: ProgressEvent<FileReader>) => {
        try {
          ...
          // 필수 필드 확인 (제품명, 모델명, 일시불 단가)
          if (parsedData.length > 0) {
            const firstRow = parsedData[0];
            const hasRequiredFields = 
              (firstRow['제품명'] !== undefined || firstRow['productName'] !== undefined) &&
              (firstRow['모델명'] !== undefined || firstRow['modelName'] !== undefined) &&
              (firstRow['일시불단가'] !== undefined || firstRow['price'] !== undefined);
            
            if (!hasRequiredFields) {
              reject(new Error('파일에 필요한 필드(제품명, 모델명, 일시불 단가)가 없습니다.'));
              return;
            }
          }
          ...
          const standardizedData: Product[] = parsedData.map(item => ({
            productName: item['제품명'] || item['productName'] || '',
            modelName: item['모델명'] || item['modelName'] || '',
            price: parseFloat(item['일시불단가'] || item['price'] || 0)
          }));
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {calculatedProducts.map((product, index) => (
                        <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{Math.round(product.supplyPrice).toLocaleString()}원</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{Math.round(product.totalRentalFee).toLocaleString()}원</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-blue-700">{product.monthlyRentalFee.toLocaleString()}원</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div className="bg-white p-4 rounded-xl shadow-md border border-gray-100">
                  ...
                  <p className="text-2xl font-bold text-gray-800">
                    {(calculatedProducts.reduce((sum, product) => sum + product.totalRentalFee, 0) / calculatedProducts.length).toLocaleString()}원
                  </p>
                </div>
                <div className="bg-white p-4 rounded-xl shadow-md border border-gray-100">
                  <h4 className="text-sm font-medium text-gray-500 mb-1">평균 월 렌탈료</h4>
                  <p className="text-2xl font-bold text-blue-600">
                    {(calculatedProducts.reduce((sum, product) => sum + product.monthlyRentalFee, 0) / calculatedProducts.length).toLocaleString()}원
                  </p>
                </div>
              </div>
